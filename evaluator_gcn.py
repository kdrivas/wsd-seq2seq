...
class Evaluator():
    def __init__(self, encoder, decoder, gcn, input_lang, output_lang, max_length, USE_CUDA):
    ...
    def evaluate(self, sentence, input_matrix, k_beams):
        ...
        encoder_outputs = nn.ReLU()(gcn(encoder_outputs.squeeze(1), input_matrix).unsqueeze(1))        
    ...
    def evaluate_acc(self, pairs, syntax_input, k_beams=3, verbose=False):
        hint = 0
        for ix, pair in enumerate(pairs):
            temp = pair.split('\t')
            sentence, pos, id_senses = temp[0], int(temp[1]), temp[2:]
            
            output_words = self.evaluate(sentence, input_matrix, k_beams)
